...
const IntigatorType = () => {
      ...
      const handleSubmit = async (e) => {
        e.preventDefault();
        if (!unitType?.trim() && unitType?.length === 0) {
          toast.error("Please enter a valid Indicator Type");
          return;
        }
        setIsLoading(true);
        try {
          const payload = {
            ...
            "lookup_value": unitType,
            "lookup_type": "station_indicator_type",
            "parent_lookup_type": "",
            "parent_lookup_id": null
          }
          const res = await __postApiData('/api/v1/admin/SaveLookup', payload);
          if (res.response && res.response.response_code === "200") {
            toast.success(editId ? "Indicator Type updated successfully" : "Indicator Type added successfully");
            setUnitType("");
            setEditId(null);
            setIsLoading(false);
            fetchData(['station_indicator_type']);
          } else {
            toast.error(res.response.response_message || "Failed to add Indicator Type");
          }
  ...
  return (
   <div className="p-4 bg-white">
      <SectionHeader
        title="Enter Details for Indicator Type"
        description="Add or update the required details for the Indicator Type to keep records accurate and complete."
      />
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-8 shadow-card  rounded-md p-4">
        ...
        <div className="mt-4">
          <FormButton disabled={isLoading}> {isLoading ? (editId ? "Updating..." : "Adding...") : editId ? "Update Indicator Type" : "Add Indicator Type"}</FormButton>
        </div>
      </form>
