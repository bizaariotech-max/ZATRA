...
const UnitType = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!unitType?.trim() && unitType?.length === 0) {
      toast.error("Please enter a valid Unit Type");
      return;
    }
    setIsLoading(true);
    try {
      ...
      if (res.response && res.response.response_code === "200") {
        toast.success(editId ? "Unit Type updated successfully" : "Unit Type added successfully");
        setUnitType("");
        setEditId(null);
        setIsLoading(false);
        fetchData(['unit_type']);
      } else {
        toast.error(res.response.response_message || "Failed to add Unit Type");
      }
  ...
  return (
    <div className="p-4 bg-white">
      <SectionHeader
        title="Enter Details for Unit Type"
        description="Add or update the required details for the unit type to keep records accurate and complete."
      />
