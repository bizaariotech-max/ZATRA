...
const CityStation = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData?.subCategoryName?.trim() && formData?.subCategoryName?.length === 0) {
            toast.error("Please enter a valid City / Station name");
            return;
        }
        setIsLoading(true);
        try {
            ...
            if (res.response && res.response.response_code === "200") {
                toast.success(editId ? "City / Station updated successfully" : "City / Station added successfully");
                setFormData({
                    subCategoryName: '',
                    categoryId: '',
                });
                ...
                fetchData(['station'], "subCategoryList");
            } else {
                toast.error(res.response.response_message || "Failed to add City / Station");
            }
            setIsLoading(false);
        } catch (error) {
            setIsLoading(false);
        ...
        try {
            ...
            if (result.isConfirmed) {
                ...
                if (res?.response?.response_code === "200") {
                    toast.success("City / Station deleted successfully");
                    fetchData(['station'], "subCategoryList");
                }
            }
        } catch (error) {
    ...
    return (
        <div className="p-4 bg-white">
            <SectionHeader
                title="Enter Details for City / Station name"
                description="Add or update the required details for the City / Station name to keep records accurate and complete."
            />
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-8 shadow-card  rounded-md p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <FormInput
                        label="City / Station name"
                        name="subCategoryName"
                        placeholder="Enter City / Station name"
